...
검색엔진들이 잘 해주고 있지만 많이 거론되는 SEO를 최대한 보장받고 싶었다. 두 번째로는 마케팅이나 외부 플랫폼에 공유했을 때에 SPA처럼 무조건 하나의 메타 정보만을 보여 주어서는
안 되었다. node 서버나 infra에 대해서 어느 정도 노하우도 조금 생기고 익숙해져 갈 무렵, 어느 정도 리서치와 회의 끝에 이 글에서 설명할 첫 번째 방법으로의 변화를 시도해보기로 결정했다.
...
frontend 빌드된 결과물은 s3에 올려두고 cloudFront에서 결국 서빙하는 것이 더 빠른 개발과 배포에 더 좋다고 생각한다. 실제로 간단히 그림을 그려 보면 다음과 같다.

![nginx-react-prerender](../assets/first-react-ssr-diagram.png)

&nbsp; 실제 infra 구성까진 한 것은 아니지만, client와 server를 나누어서 직접 로컬에서 간단히 작동하는 수준까지만 작업해보았다.
연습 삼아 작성해본 코드는 위에 링크 혹은 이 [repository](https://github.com/impressor615/nginx-react-prerender)링크에서 확인할 수 있다. client라는 폴더에는
...
커뮤니티는 점점 더 활발해지고 심지어 next.js는 진짜 빨리 업데이트가 된다. 그래서 진짜 개인적으로 좋아 한다. 결국 내가 문제였던 것으로.
다시 next.js를 사용한다면 cache를 세심하게 다루고 최근에 next.js에서 도입된 [Automatic Static Optimization](https://nextjs.org/docs/advanced-features/automatic-static-optimization)를
잘 이용할 수 있도록 어플리케이션을 구성했다면 더 안정성있게 구현하지 않을까 아쉬움이 남기도 한다. 사실 이 기능을 사용하면 next.js를 사용하는 것과 위의 말했던 방식과 거의 비슷하게 사용할 수 있긴 하다.
무엇이든 유일한 답이란 없고 자신이 풀어야 하는 문제에 가장 합리적인 방법을 선택해야하는 것 뿐이지만. 마지막으로 nginx를 활용한 SSR방법에 대한 장단점을 간단히 정리해보면 다음과 같지 않을까 싶다. 
...
&nbsp; 마지막으로 회사에서는 이 구성에 대해서 다른 동료 분이 구현해주셔서 직접 구현하지 않았었지만, 구현하시는 모습을 보면서 공부하는 마음으로
직접 구현을 해봐야 겠다는 마음을 들게 만들어 주시고 항상 좋은 조언과 에너지를 나눠 주시는 동료 분에게도 감사하는 마음을 전한다.
